...
__attribute__((unused)) static int memfd_create(const char *name,
                                                unsigned int flags) {
...
#define __NR_memfd_create 279
  ...
  return syscall(__NR_memfd_create, name,
                 flags); // NOLINT(cppcoreguidelines-narrowing-conversions)
#else
  errno = ENOSYS;
  return -1;
#endif
...
__LIBC_HIDDEN__ LorieBuffer *LorieBuffer_allocate(int32_t width, int32_t height,
  ...
  *buffer = (LorieBuffer){.desc = {.width = width,
                                   .stride = width,
                                   .height = height,
                                   .format = format,
                                   .type = type},
  ...
  if (type == LORIEBUFFER_REGULAR) {
    ...
    AHardwareBuffer_Desc desc = {.width = width,
                                 .height = height,
                                 .format = format,
                                 .layers = 1,
                                 .usage =
    ...
    if (err != 0) {
      dprintf(2,
              "FATAL: failed to allocate AHardwareBuffer (width %d height %d "
              "format %d): error %d\n",
  ...
  if (buffer->desc.type == LORIEBUFFER_REGULAR) {
    if (buffer->desc.data)
      munmap(buffer->desc.data,
             alignToPage(buffer->desc.width * buffer->desc.height *
                         sizeof(uint32_t)));
    if (buffer->fd != -1)
      close(buffer->fd);
...
__LIBC_HIDDEN__ void LorieBuffer_describe(LorieBuffer *buffer,
                                          LorieBuffer_Desc *desc) {
  if (!desc)
    return;
...
__LIBC_HIDDEN__ int LorieBuffer_lock(LorieBuffer *buffer,
...
__LIBC_HIDDEN__ int LorieBuffer_copy(LorieBuffer *src, LorieBuffer *dst) {
  int ret = 0;
  uint8_t srcWasLocked = src ? src->locked : false,
          dstWasLocked = dst ? dst->locked : false;
  ...
  ret = !pixman_blt(src->lockedData, dst->lockedData, (int)src->desc.stride,
                    (int)dst->desc.stride, 32, 32, 0, 0, 0, 0,
                    (int)min(src->desc.width, dst->desc.width),
                    (int)min(src->desc.height, dst->desc.height));
...
LorieBuffer_sendHandleToUnixSocket(LorieBuffer *_Nonnull buffer, int socketFd) {
...
LorieBuffer_recvHandleFromUnixSocket(int socketFd, LorieBuffer **outBuffer) {
  ...
  // We should read buffer from socket despite outbuffer is NULL, otherwise we
  // will get protocol error
  ...
  buffer.fd = -1;
  buffer.lockedData = NULL;
  __sync_fetch_and_add(&buffer.refcount,
...
__LIBC_HIDDEN__ void LorieBuffer_importFD(int fd, int32_t width, int32_t height,
                                          int32_t stride, int8_t format,
                                          LorieBuffer **outBuffer) {
  if (fd < 0 || !outBuffer)
    return;
  ...
  buffer->desc.type = LORIEBUFFER_AHARDWAREBUFFER;
  buffer->fd = -1;

  // Use socketpair trick to import AHardwareBuffer from FD if possible
  // Actually, DRI3 usually passes a DMA-BUF FD.
  // For AHardwareBuffer, we need to wrap it.
  // On Android, a DMA-BUF FD *is* what AHardwareBuffer_sendHandleToUnixSocket
  // ultimately sends. But we need to use the Android APIs to get the
  // AHardwareBuffer object back.

  int sv[2];
  if (socketpair(AF_UNIX, SOCK_STREAM, 0, sv) == 0) {
    ancil_send_fd(sv[0], fd);
    AHardwareBuffer_recvHandleFromUnixSocket(sv[1], &buffer->desc.buffer);
    close(sv[0]);
    close(sv[1]);
  }
...
__LIBC_HIDDEN__ int LorieBuffer_getFD(LorieBuffer *buffer) {
  if (!buffer)
    return -1;
  ...
  if (buffer->desc.type == LORIEBUFFER_AHARDWAREBUFFER && buffer->desc.buffer) {
    ...
    int fd = -1;
    if (socketpair(AF_UNIX, SOCK_STREAM, 0, sv) == 0) {
      AHardwareBuffer_sendHandleToUnixSocket(buffer->desc.buffer, sv[0]);
      fd = ancil_recv_fd(sv[1]);
      close(sv[0]);
      close(sv[1]);
    }
    return fd;
...
__LIBC_HIDDEN__ int ancil_send_fd(int sock, int fd) {
  ...
  struct msghdr message_header = {.msg_name = NULL,
                                  .msg_namelen = 0,
                                  .msg_iov = &nothing_ptr,
                                  .msg_iovlen = 1,
                                  .msg_flags = 0,
